...
from btp.visualization.analysis import load_all_features, visualize_tsne, visualize_similarity_heatmap, visualize_single_feature_vector, visualize_before_vs_after
...
def main():
    ...
    event_process_output = working_dir / "Preprocessed-Dataset" / "Processed_Lowlight_event"

    if event_process_input.exists():
        processor = BatchEventProcessor(voxel_size=(5, 5), time_window=1e3, density_threshold=2)
        processor.process_all_event_files(input_dir=event_process_input, output_dir=event_process_output)
    else:
        print(f"Skipping event preprocessing: Input directory {event_process_input} does not exist.")

    # 3. Image Data Preprocessing
    print("\n--- 3. Image Data Preprocessing ---")
    input_image_process_path = raw_data_path / "Dataset" / "Lowlight_Images"
    output_image_process_path = working_dir / "Preprocessed-Dataset" / "Processed_Lowlight_Images"

    if input_image_process_path.exists():
        image_processor = ImageProcessor()
        image_processor.process_all_images(input_image_process_path, output_image_process_path)
    else:
        print(f"Skipping image preprocessing: Input directory {input_image_process_path} does not exist.")
        ...
        if sample_files:
            ...
            try:
                ...
                if raw_file.exists():
                    ...
                    EventComparator.visualize_raw_vs_processed(raw_file, processed_file, output_dir=output_viz_dir)
    ...
    if input_image_process_path.exists() and output_image_process_path.exists():
        ...
        try:
            ImageVisualizer.visualize_preprocessing(input_image_process_path, output_image_process_path, num_examples=5, save_dir=save_dir)
            ImageVisualizer.plot_histograms(input_image_process_path, output_image_process_path, num_examples=5, save_dir=save_dir)
        except Exception as e:
    ...
    extract_event_output_dir_base = working_dir / "Extracted-Dataset" / "Extracted_Lowlight_event"
    ...
    extract_image_output_dir = working_dir / "Extracted-Dataset" / "Extracted_Lowlight_Images"
    ...
    if not image_feat_input_dir.exists() and preprocessed_data_path:
         image_feat_input_dir = preprocessed_data_path / "Preprocessed-Dataset" / "Processed_Lowlight_Images"
    ...
    if fused_output_root.exists():
        features, labels, folder_map = load_all_features(str(fused_output_root), max_folders=10)
        if features.size > 0:
            try:
